      ...
      <div className={`login-container ${darkMode ? 'dark' : ''}`}>
        ...
        <div className="login-features">
          <div className="feature">
            ...
            {grupoSeleccionado && (
              <div style={{ marginTop: '10px' }}>
                <h4>Agregar contacto al grupo</h4>
                <input
                  type="text"
                ...
                />
                <button onClick={() => agregarContactoAGrupo(nuevoNumero, nombreGrupo)}>Agregar al grupo</button>
              </div>
            )}
            ...
            {grupoSeleccionado && (
              <div style={{ marginTop: '20px' }}>
                <h4>Mensajes en {grupoSeleccionado}</h4>
                <div style={{
                  maxHeight: '200px',
                  overflowY: 'auto',
                  ...
                  {grupos[grupoSeleccionado]?.mensajes?.map((msg, i) => (
                    <div key={i}>
                      <strong>{msg.autor}</strong> ({msg.fecha}): {msg.texto}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ marginTop: '20px' }}>
              ...
              <ul>
                {Object.values(contactos).map((c, i) => (
                  <li key={i}>{c.nombre} ({c.numero})</li>
                ))}
              </ul>
            </div>
          </div>
      ...
      <div className="main-content">
        ...
        <div className={`sidebar ${showSidebar ? 'visible' : ''} ${showMobileUserList ? 'mobile-visible' : ''}`}>
          ...
          <div className="chat-tabs">
            <button className="active">Conversaciones</button>
        ...
        <div className="chat-main">
          ...
          <div className="chat-info">
            <div className="chat-info-left">
              <h3>
                {selectedUser === 'all' ? 'Chat General' : 
                 isGroupChat ? currentGroup?.name : 
                 getSafeUserName(users.find(u => u.id === selectedUser))}
              </h3>
              
              <div className="chat-status">
                {selectedUser === 'all' ? `${users.length + 1} participantes` : 
                 isGroupChat ? `${currentGroup?.members.length} miembros` : 
                 users.find(u => u.id === selectedUser)?.status === 'online' ? 'En línea' : 
                 `Últ. vez ${formatDistanceToNow(new Date(users.find(u => u.id === selectedUser)?.lastSeen), { addSuffix: true, locale: es })}`}
              </div>
            </div>
          ...
          <div className="message-input-area">
            ...
            <div className="message-input-wrapper">
              <textarea
                ref={messageInputRef}
                placeholder={`Escribe un mensaje ${selectedUser === 'all' ? 'al grupo' : isGroupChat ? 'al grupo' : 'privado'}`}
                ...
                {showUserProfile !== socket.id && (
                  <>
                    <button 
                      ...
                      onClick={() => {
                        setSelectedUser(showUserProfile);
